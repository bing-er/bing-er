...
|----------|--------------|-------------|
| 🔦 [Monte-Carlo Photon Transport (Jacques Model)](https://github.com/bing-er/mc-forward-jacques) | High-performance forward Monte Carlo solver for photon transport in turbid media, based on the Jacques MCML framework. Implements HOP–DROP–SPIN–ROULETTE propagation with Henyey–Greenstein scattering, Fresnel boundary conditions, and multi-geometry fluence scoring. Refactored with Numba JIT parallelism to eliminate the forward solver as a computational bottleneck (≈500× speedup), enabling feasibility studies for large-scale inverse and imaging problems. | Python, NumPy, Physics Modeling, LaTeX |
